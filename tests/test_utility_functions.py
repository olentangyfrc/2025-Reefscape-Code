...
def test_convert_ticks_to_degrees(tick_count:int, expected:float) -> None:
    assert utility_functions.convert_ticks_to_degrees(tick_count) == pytest.approx(expected)
...
def test_convert_ticks_to_radians(tick_count:int, expected:float) -> None:
    assert utility_functions.convert_ticks_to_radians(tick_count) == pytest.approx(expected)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Pose2d(0, 0, Rotation2d(math.radians(5))), Pose2d(0, 0, Rotation2d(math.radians(0))), 1, 10, True),
        (Pose2d(0, 0, Rotation2d(math.radians(15))), Pose2d(0, 0, Rotation2d(math.radians(0))), 1, 10, False),
        (Pose2d(0, 0, Rotation2d(0)), Pose2d(1, 1, Rotation2d(0)), 2, 10, True),
        (Pose2d(0, 0, Rotation2d(0)), Pose2d(1, 1, Rotation2d(0)), 1, 10, False),
    ],
)
def test_within_pose_tolerance(p1:Pose2d, p2:Pose2d, trans_tol:float, rot_tol:float, expected:bool) -> None:
    assert utility_functions.within_pose_tolerance(p1, p2, trans_tol, rot_tol) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Translation2d(FIELD_LENGTH, FIELD_WIDTH), Translation2d(-FIELD_LENGTH / 2, -FIELD_WIDTH / 2)),
        
        (Translation2d(FIELD_LENGTH / 2, FIELD_WIDTH), Translation2d(0, -FIELD_WIDTH / 2)),
        
        (Translation2d(FIELD_LENGTH / 4, FIELD_WIDTH / 4), Translation2d(-FIELD_LENGTH / 4 + FIELD_LENGTH / 2, -FIELD_WIDTH / 4 + FIELD_WIDTH / 2)),
        (Translation2d(FIELD_LENGTH * 0.75, FIELD_WIDTH * 0.75), Translation2d(-FIELD_LENGTH * 0.75 + FIELD_LENGTH / 2, -FIELD_WIDTH * 0.75 + FIELD_WIDTH / 2)),
    ]
)
def test_red_to_blue(red_position, expected_blue_position):
    
    blue_position = utility_functions.red_to_blue(red_position)
    assert blue_position.X() == expected_blue_position.X(), f"Expected X to be {expected_blue_position.X()}, but got {blue_position.X()}"
    assert blue_position.Y() == expected_blue_position.Y(), f"Expected Y to be {expected_blue_position.Y()}, but got {blue_position.Y()}"