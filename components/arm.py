...
class Arm:
    ...
    def __init__(self) -> None:
        ...
        self.intake_roller_configs.motor_output.neutral_mode = (
            signals.NeutralModeValue.BRAKE
        )
        self.intake_roller_configs.current_limits.supply_current_limit = 20
        self.intake_roller_configs.motor_output.inverted = (
            signals.InvertedValue.COUNTER_CLOCKWISE_POSITIVE
        )

        self.intake_roller_motor = hardware.TalonFX(constants.ARM_ROLLERS_ID, "*")
        ...
        self.intake_angle_configs.motor_output.neutral_mode = (
            signals.NeutralModeValue.BRAKE
        )
        self.intake_angle_configs.current_limits.supply_current_limit = 50
        self.intake_angle_configs.motor_output.inverted = (
            signals.InvertedValue.COUNTER_CLOCKWISE_POSITIVE
        )

        self.intake_angle_motor = hardware.TalonFX(constants.ARM_PIVOT_ID, "*")
        self.intake_angle_motor.configurator.apply(self.intake_angle_configs)

        # Encoder
        self.angle_absolute_encoder = wpilib.DutyCycleEncoder(1, 1, -0.214)
        ...
        canConfigs = (
            ...
            .with_fov_params(fov)
            .with_proximity_params(proximity)
            .with_to_f_params(tof)
        )
    ...
    def setup(self) -> None:
        ...
        self.intake_angle_pid = ProfiledPIDController(
            self.p, self.i, self.d, self.constraints
        )
        ...
        self.intake_angle_ff = ArmFeedforward(self.ks, self.kg, 0.0)
    ...
    def get_filtered_current(self) -> float:
        ...
        """
        return self.linear_filter.calculate(
    ...
    def get_pivot_position(self) -> float:
        ...
        """
        if RobotBase.isReal():
            return utils.inputModulus(
                self.angle_absolute_encoder.get() * math.tau, -math.pi, math.pi
            )
    ...
    def execute(self):
        ...
        if self.manual:
            ...
            self.target_angle = math.radians(
                utils.clamp(self.manual_angle, self.MINIMUM_ANGLE, self.MAXIMUM_ANGLE)
            )
        ...
        pid_output = utils.clamp(
            pid_output,
            -self.voltage_ramp_timer.get() * 7,
            self.voltage_ramp_timer.get() * 7,
        )
        ff_output = self.intake_angle_ff.calculate(
