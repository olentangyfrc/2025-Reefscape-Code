...
class EndEffector:
    ...
    def __init__(self) -> None:
        ...
        manipulator_motor_config.motor_output.neutral_mode = signals.NeutralModeValue.COAST
        manipulator_motor_config.current_limits.stator_current_limit_enable = True
        manipulator_motor_config.current_limits.stator_current_limit = EndEffector.CURRENT_LIMIT

        self.manipulator_motor = hardware.TalonFX(
            constants.END_EFFECTOR_ID, "*")
        ...
        canrange_configs = (configs.CANrangeConfiguration()
                            .with_fov_params(fov_params)
                            .with_proximity_params(proximity_params)
                            .with_to_f_params(tof_params)
        )

        self.exit_canrange = hardware.CANrange(
            constants.CORAL_EXIT_ID, "*")
        self.entry_canrange = hardware.CANrange(
            constants.CORAL_ENTRY_ID, "*")
        ...
        self.state = EndEffectorStates.HOLDING if self.ready() else EndEffectorStates.IDLE
    ...
    @feedback(key="End effector current")
    ...
    def execute(self) -> None:
        ...
        match self.state:
            ...
            case EndEffectorStates.INTAKING:
                ...
                if self.get_entry():
                    if self.get_exit():
                        self.manipulator_motor.set_control(controls.VoltageOut(self.min_speed))
                    else:
                        self.manipulator_motor.set_control(controls.VoltageOut(self.slow_speed))
                elif not self.get_entry() and self.get_exit():
                    self.holding_coral = True
                    self.state = EndEffectorStates.REVERSING
                    self.manipulator_motor.set_control(controls.VoltageOut(0))
                elif not (self.get_entry() and self.get_exit()):
                    self.manipulator_motor.set_control(controls.VoltageOut(self.max_speed))
            ...
            case EndEffectorStates.DISPENSING:
                ...
                if not self.get_entry() and not self.get_exit():
                    ...
                    self.manipulator_motor.set_control(controls.VoltageOut(EndEffector.RELEASE_SPEED))
