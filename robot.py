...
class MyRobot(magicbot.MagicRobot):
    ...
    enable_offset = tunable(
        True
    )  # This was a quck solution at champs to an issue where we were consistenly missing placements to the right.
    ...
    def teleopPeriodic(self) -> None:
        ...
        self.elevator.set_arm_position(
            math.degrees(self.arm.get_pivot_position())
        )  # Send current arm angle to elevator to prevent crash scenarios
        ...
        if (
            len(self.dashboard.get_node()) > 1
        ):  # Dashboard nodes are sent with the structure [Reef letter Placement height].
            # For example: "E2", "A4", "J3"
            if (
                (self.state not in [Robot_State.CLIMB, Robot_State.LINE_UP_TO_REEF])
                and self.main_controller.getRightBumper()
                and self.end_effector.get_exit()
            ):
                ...
                if rf.can_line_up(self.drivetrain.get_pose().translation(), node[0]):
                    if self.enable_line_up:
                        self.drivetrain.line_up(
                            Reef_Position.from_string(node[0]).transformBy(
                                Transform2d(0, 0.02 * self.enable_offset, 0)
                            )
                        )
                    self.elevator.go_to(ElevatorState.from_int(int(node[1])))
        ...
        if self.main_controller.getAButtonPressed():
            if (
                (
                    not self.arm.holding_algae
                    or (not RobotBase.isReal() and self.arm.sim_holding)
                )
                and self.drivetrain.is_manual()
                and self.state != Robot_State.CLIMB
            ):
        ...
        if self.main_controller.getBButtonPressed():
            ...
            if self.end_effector.ready() and self.arm.get_pivot_position_deg() > 80:
                if self.main_controller.getLeftTriggerAxis() > 0.5:
                    ...
                    self.drivetrain.line_up(
                        constants.L1_End_Effector_Reef_Position.from_string(
                            rf.nearest_reef_branch(
                                self.drivetrain.get_pose().translation()
                                + self.drivetrain.get_velocity().translation() * 0.15
                            )
                        )
                    )
        ...
        if self.main_controller.getYButtonPressed():
            if self.arm.state == ArmStates.HOLD_ALGAE:
            ...
            elif self.arm.state in [
                ArmStates.PREPARE_REMOVE_ALGAE,
                ArmStates.REMOVE_ALGAE,
                ArmStates.IDLE,
            ]:
                ...
                if self.drivetrain.get_pose().X() < constants.FIELD_LENGTH / 2:
                    ...
                    blue_loc = Translation2d(
                        constants.FIELD_LENGTH - self.drivetrain.get_pose().X(),
                        constants.FIELD_WIDTH - self.drivetrain.get_pose().Y(),
                    )
                    blue_target = rf.get_algae_loc(rf.nearest_reef_branch(blue_loc))
                    self.drivetrain.line_up(
                        Pose2d(
                            constants.FIELD_LENGTH - blue_target.X(),
                            constants.FIELD_WIDTH - blue_target.Y(),
                            -blue_target.rotation(),
                        )
                    )
                    relative_translation = Translation2d(
                        constants.FIELD_LENGTH - relative_translation.X(),
                        constants.FIELD_WIDTH - relative_translation.Y(),
                    )
        ...
        if self.main_controller.getXButtonPressed():
            if self.state not in [
                Robot_State.LINE_UP_TO_REEF,
                Robot_State.L1_DROP,
                Robot_State.L1_STACK,
            ]:
                if self.enable_line_up:
                    # Find the branch that the bot will be closest to after a fraction of a second
                    self.drivetrain.line_up(
                        Reef_Position.from_string(
                            rf.nearest_reef_branch(
                                self.drivetrain.get_pose().translation()
                                + self.drivetrain.get_velocity().translation() * 0.15
                            )
                        ).transformBy(Transform2d(0, 0.02 * self.enable_offset, 0))
                    )
                self.state = Robot_State.LINE_UP_TO_REEF
        ...
        if (
            self.main_controller.getRightTriggerAxis() > 0.5
            and self.arm.get_pivot_position_deg() > 80
        ):
            self.state = Robot_State.STATION_INTAKE
            if self.enable_line_up:
                y = min(
                    self.drivetrain.get_pose().Y(),
                    constants.FIELD_WIDTH - self.drivetrain.get_pose().Y(),
                )
                ...
                if self.drivetrain.get_pose().Y() > constants.FIELD_WIDTH / 2:
                    self.drivetrain.line_up(
                        Pose2d(
                            position.X(),
                            constants.FIELD_WIDTH - position.Y(),
                            Rotation2d(-0.94),
                        )
                    )
        ...
        if (
            self.state == Robot_State.STATION_INTAKE
            and self.main_controller.getRightTriggerAxis() < 0.5
    
            and self.arm.get_pivot_position() * 180 / math.pi > 90
        ):
            self.end_effector.intake()
            self.drivetrain.manual_drive()
        ...
        """Aux Controls"""
        if self.aux_controller.getAButton() or (
            self.main_controller.getLeftTriggerAxis() > 0.9
            and self.main_controller.getPOV(0) == 90
        ):
            if (
                self.drivetrain.get_pose().X() > 6.5
                or DriverStation.getMatchTime() < 20.0
                or not self.enable_line_up
                or self.aux_controller.getBButton()
            ):
        ...
        match self.state:
            ...
            case Robot_State.LINE_UP_TO_REEF:
                if (
                    ...
                    and self.elevator.at_target()
                    and self.elevator.get_height() > 0.1
                    and self.end_effector.ready()
                ):
                    self.end_effector.dispense()
            ...
            case Robot_State.CLIMB:
                ...
                if self.aux_controller.getXButton() or (
                    self.main_controller.getAButton()
                ):
                    self.climber.climb()
            ...
            case Robot_State.DEALGIFY_REEF:
                if (
                    self.drivetrain.is_aligned()
                    or self.drivetrain.state != DrivetrainState.LINE_UP_TELEOP
                    or self.arm.is_holding()
                ):
                    self.drivetrain.manual_drive()
                ...
                if not self.arm.holding_algae:
                    if (
                        rf.nearest_reef_branch(self.drivetrain.get_pose().translation())
                        in "abefij"
                    ):
                        self.elevator.set(ElevatorState.ALGAE_HIGH)
                    else:
            ...
            case Robot_State.PROCESSOR:
                if self.arm.holding_algae or (
                    not RobotBase.isReal() and self.arm.sim_holding
                ):
                    self.elevator.set(ElevatorState.STOW)
                    if self.elevator.at_target() and (
                        self.drivetrain.state == DrivetrainState.ALIGNED
                        or self.drivetrain.get_pose()
                        .translation()
                ...
                if self.arm.holding_algae:
                    ...
                    if math.degrees(self.arm.get_pivot_position()) > 65:
                        ...
                        if not self.backup_timer.isRunning():
                            self.backup_timer.restart()
                        if (
                            self.drivetrain.get_signal().get_speed().vx < -0.5
                            or self.backup_timer.get() > 1
                        ):
                            self.backup_timer.stop()
                            self.backup_timer.reset()
                            self.elevator.set(ElevatorState.STOW)
            ...
            case Robot_State.L1_DROP:
                ...
                if (
                    self.drivetrain.state == DrivetrainState.ALIGNED
                    and self.elevator.at_target()
                    and not self.backup_timer.isRunning()
                ):
                ...
                if self.backup_timer.get() > 0.4:
                    ...
                    for i in [
                        ...
                        if (
                            ...
                            new_target = i.value
                            new_target = new_target.transformBy(
                                Transform2d(-0.44, 0, 0)
                            )
                    self.drivetrain.line_up(new_target)
                    self.end_effector.stop()

                    current = rf.nearest_reef_branch(
                        self.drivetrain.get_pose().translation()
                    ).capitalize()
                    conjugate = constants.L1_End_Effector_Reef_Position.from_string(
                        "BADCFEHGJILK"["ABCDEFGHIJKL".index(current)]
                    )
                    self.drivetrain.line_up(conjugate)

            case Robot_State.L1_SWIPE:
                if (
                    self.drivetrain.target_pose.translation().distance(
                        self.drivetrain.get_pose().translation()
                    )
                    < 0.3
                ):
            ...
            case Robot_State.L1_STACK:
                if not self.end_effector.ready() or not RobotBase.isReal():
                    ...
                    if self.backup_timer.get() > 0.25:
                        if self.l1_pop_index is not None:
    ...
    def drive_with_joystick(self, field_relative: bool) -> None:
        ...
        if self.drivetrain.is_manual():
            x_speed = (
                -utility_functions.filter_input(self.main_controller.getLeftY())
                * constants.MAX_LINEAR_SPEED
                * modifier
            )
            y_speed = (
                -utility_functions.filter_input(self.main_controller.getLeftX())
                * constants.MAX_LINEAR_SPEED
                * modifier
            )
            rot = (
                -utility_functions.filter_input(self.main_controller.getRightX())
    ...
    def cancel_all(self):
        ...
        if self.state == Robot_State.NET and self.arm.is_holding():
            ...
            math.degrees(self.arm.get_pivot_position()) >= -60
            or self.drivetrain.is_manual()
        ):
