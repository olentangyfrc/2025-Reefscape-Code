...
class BaseAutonomous(AutonomousStateMachine):
    ...
    def __init__(self) -> None:
        ...
        try:
            self.base_trajectories: dict[str, SwerveTrajectory] = {
                path: choreo.load_swerve_trajectory(base_dir + path) for path in [
                    "B-RightStation",
                    "C-RightStation",
                    "D-RightStation",
                    "E-RightStation",
    ...
    def on_enable(self) -> None:
        ...
        if dashboard_auton[0] == "PUSH-AUTON":
            ...
            self.actions = [constants.Auton_Actions.REEF_SCORE, constants.Auton_Actions.INTAKE_CORAL] * 3
            self.trajectories = [
        ...
        # Handle other cases
            ...
            self.drivetrain.reset_odometry(Pose2d(7.2, constants.FIELD_WIDTH-2.42, math.pi))
        
        elif dashboard_auton[0] == "SIMPLE-RIGHT":
            self.drivetrain.reset_odometry(Pose2d(7.2, constants.FIELD_WIDTH/2, math.pi))
        ...
        # Handle custom auton sequences
            ...
            start_pose = Pose2d(7.2, constants.FIELD_WIDTH-2.42 if is_flipped else 2.42, math.pi)
    ...
    def pid_drive(self, initial_call) -> None:
        ...
        if self.drivetrain.get_pose().translation().distance(self.drivetrain.target_pose.translation()) < 0.04 and not self.tripped:
            self.elevator.set(ElevatorState.L4)

        if self.elevator.get_height() > 1.5 and self.timer.get() > 3.0 and not self.tripped:
    ...
    def intake_coral(self, initial_call) -> None:
        ...
        if self.end_effector.ready() or (self.intake_timer.get() > 0.15 and RobotBase.isReal()):
    ...
    def score_on_reef(self, initial_call) -> None:
        ...
        if trajectory:
            ...
            if sample:
                augmented = self.augment_sample(sample, 10, self.flipped_list[self.current_path_index])
                self.drivetrain.follow_trajectory(augmented, trajectory)
                self.drivetrain.target_pose = Pose2d(augmented.x, augmented.y, augmented.heading)
    ...
    def follow_trajectory(self, initial_call) -> None:
        ...
        if initial_call:
            self.timer.restart()
            if self.trajectories[self.current_path_index] and self.trajectories[self.current_path_index].sample_at(10, False):
                fin: SwerveSample = self.augment_sample(self.trajectories[self.current_path_index].sample_at(10, False), 10, self.flipped_list[self.current_path_index])
        ...
        if trajectory.get_total_time()-time < ELEVATOR_UP_TIME and action == constants.Auton_Actions.REEF_SCORE and (self.end_effector.get_exit() or self.current_path_index == 0):
            self.elevator.set(ElevatorState.L4)
        elif self.end_effector.get_exit() and action == constants.Auton_Actions.REEF_SCORE:
        ...
        elif time > trajectory.get_total_time() + GIVE_UP_TIME and action == constants.Auton_Actions.REEF_SCORE:
            self.next_state_now("score_on_reef")
    ...
    def pid_to_G(self, initial_call) -> None: 
        ...
        if self.elevator.get_height() > 1.5 and self.drivetrain.is_aligned() and not self.tripped:
            self.end_effector.dispense()
            self.timer.restart()
            self.tripped = True
    ...
    def augment_sample(self, sample: SwerveSample, time: float, flipped: bool) -> SwerveSample:
        augmented_sample = SwerveSample(time, sample.x, sample.y, sample.heading, sample.vx,
                                      sample.vy, sample.omega, sample.ax, sample.ay, sample.alpha, sample.fx, sample.fy)
        if flipped:
    ...
    def is_aligned(self, sample_pose) -> bool:
        return utils.within_pose_tolerance(self.drivetrain.get_pose(), sample_pose, 0.05, 1)