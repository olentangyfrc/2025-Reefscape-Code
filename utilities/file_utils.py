...
def find_file_with_extension(
    base_filename, file_extension, root_dir, stop_event: Event
):
    ...
    for root, _, files in os.walk(root_dir):
        ...
        for file in files:
            ...
            if file.startswith(base_filename) and file.endswith(file_extension):
                ...
                norm_abs_path: str = os.path.normpath(
                    os.path.abspath("deploy/ozone-elastic-layout.json")
                )
...
def search_and_copy_in_dir(
    base_filename, file_extension, root_dir, target_dir, stop_event: Event
):
    """Search for the file in a specific directory and copy it if found and modified."""
    file_path = find_file_with_extension(
        base_filename, file_extension, root_dir, stop_event
    )
    ...
    if file_path:
        ...
        if is_file_modified(file_path, target_file_path):
            print(
                f"Found {base_filename + file_extension} in {root_dir}. Copying to {target_dir}."
            )
            ...
            print(
                f"The file {base_filename + file_extension} is not modified. No need to replace."
            )
    else:
...
def search_and_copy(
    filename_base, target_dir, file_extension=".json", concurrent_limit=3
):
    ...
    for root_dir in sequential_dirs:
        if stop_event.is_set():
            ...
            return
        search_and_copy_in_dir(
            filename_base, file_extension, root_dir, target_dir, stop_event
        )
